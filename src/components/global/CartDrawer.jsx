...
export default function CartDrawer() {
  ...
  const subtotal = useMemo(() => {
    if (!cart) return "0.00";
    return calculateTotal(cart.lines?.edges || []);
  }, [cart]);
  ...
  // Listen for open events from anywhere (e.g., AddToCartButton)
  useEffect(() => {
              ...
              return (
                <div key={node.id} className="flex gap-3 items-start border-b pb-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600">£ {(price * node.quantity).toFixed(2)}</p>
