...
export default function BentoSection({ images }) {
  ...
  const imageMap = images.reduce((acc, edge) => {
    const position = edge.node.metafields?.find((mf) => mf?.key === 'bento_position')?.value;
    let link = edge.node.metafields?.find((mf) => mf?.key === 'banner_link')?.value || '#';
    ...
    if (mappedPosition) {
      acc[mappedPosition] = {
        src: edge.node.images.edges[0]?.node.src || '/images/placeholder.jpg',
        altText: edge.node.images.edges[0]?.node.altText || edge.node.title || 'Bento image',
        link,
      };
    } else if (position) {
      console.warn(`Invalid bento_position: ${position} for product ${edge.node.title}`);
    }
    return acc;
  }, {});
  ...
  return (
    <section className="max-w-[1400px] mx-auto gap-[10px] py-12 flex flex-col min-h-[50vh] md:min-h-[70vh] lg:min-h-[80vh]">
      ...
      <div className="w-full relative rounded-xl overflow-hidden aspect-[14/2] group">
        {imageMap['top'] ? (
          <Link href={imageMap['top'].link} aria-label={`View ${imageMap['top'].altText}`}>
            <Image
      ...
      <div className="w-full flex flex-col md:flex-row gap-[10px] h-full">
        ...
        <div className="w-full md:w-1/2 relative rounded-xl overflow-hidden aspect-square group">
          {imageMap['left'] ? (
            <Link href={imageMap['left'].link} aria-label={`View ${imageMap['left'].altText}`}>
              <Image
        ...
        <div className="w-full md:w-1/2 flex flex-col gap-[10px]">
          ...
          <div className="w-full relative rounded-xl overflow-hidden aspect-[16/6] group">
            {imageMap['top-right'] ? (
              <Link href={imageMap['top-right'].link} aria-label={`View ${imageMap['top-right'].altText}`}>
                <Image
          ...
          <div className="w-full flex flex-col sm:flex-row gap-[10px]">
            <div className="w-full sm:w-1/2 relative rounded-xl overflow-hidden aspect-[3/4] group">
              {imageMap['bottom-right-1'] ? (
                <Link href={imageMap['bottom-right-1'].link} aria-label={`View ${imageMap['bottom-right-1'].altText}`}>
                  <Image
            ...
            <div className="w-full sm:w-1/2 relative rounded-xl overflow-hidden aspect-[4/3] group">
              {imageMap['bottom-right-2'] ? (
                <Link href={imageMap['bottom-right-2'].link} aria-label={`View ${imageMap['bottom-right-2'].altText}`}>
                  <Image
