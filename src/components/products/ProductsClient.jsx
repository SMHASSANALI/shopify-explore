...
export const ProductsClient = ({ initialProducts, initialHasNextPage, initialEndCursor }) => {
  ...
  useEffect(() => {
    ...
    filtered = filtered.filter(
      (product) =>
        product.node.price >= filters.price.min &&
        product.node.price <= filters.price.max
    );
    ...
    switch (sort) {
      ...
      case "price-ascending":
        filtered.sort((a, b) => a.node.price - b.node.price);
        break;
      case "price-descending":
        filtered.sort((a, b) => b.node.price - a.node.price);
        break;
        ...
        {hasNextPage && (
          <div className="text-center mt-4">
            <button
              ...
              className="bg-[var(--primary-light)] cursor-pointer text-white px-4 py-2 rounded hover:bg-[var(--primary-dark)] disabled:bg-gray-400"
            >
